...
def quote_callback(topic: str, quote: dict):
    ...
    print(quote)  # ['Close']
    global j
    ticks = api.ticks(api.Contracts.Stocks[topic[3]], "file_date_")
    OP_td = ticks.close[0]

    with open('./' + file_date_ + '_' + str(i) + '.csv', 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([topic[3], OP_td, quote['Close']])
        j = j + 1
...
for stock in stock_list:
    if os.path.isfile('./stock_data/DATA/DATA_' + stock + '.csv'):
        ...
        if int((temp[- 1][12] + temp[- 2][12])) / 2 > 4000 and len(temp[:]) > 301:
            ...
            print(ticks.close[0])
            api.quote.subscribe(api.Contracts.Stocks[stock], quote_type='tick')
            j = 0
            while j == 0:
                time.sleep(0.5)
