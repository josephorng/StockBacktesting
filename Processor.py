...
for stock in stock_list:
    ...
    if os.path.isfile('./stock_data/RAW/' + stock + '.csv'):
        ...
        def slope_zero(arr, col_input, col_high, col_low, col_prev_high, col_prev_low):
            ...
            def avr(i_):
                return (arr[i_ - 1][col_input] + arr[i_ + 1][col_input] + arr[i_][col_input]) / 3
            ...
            low_list = []
            low_no = []
            while i < len(arr[:]) - gap - 3:
                i = i + 1
                flag = (avr(i) - avr(i - gap)) * (avr(i + gap) - avr(i))

                if flag < 0 and (avr(i) - avr(i - gap)) > 0 and high_count == 0 and low_count != 0:

                    high_list.append(arr[i][col_input])
                    high_no.append(i)
                    ...
                    i = i + 1
                    flag = (avr(i) - avr(i - gap)) * (avr(i + gap) - avr(i))
                    while flag < 0 and (avr(i) - avr(i - gap)) > 0 and high_count != 0 and i < len(arr[:]) - gap - 2:
                        high_list.append(arr[i][col_input])
                        high_no.append(i)
                        high_count = high_count + 1
                    ...
                    if high_count > 1:
                        for j in high_no:
                            if j <= high_no[high_list.index(max(high_list))]:
                                arr[j][col_high] = high
                                arr[j][col_low] = low
                                arr[j][col_prev_high] = high_prev
                                arr[j][col_prev_low] = low_prev
                ...
                elif flag < 0 and (avr(i) - avr(i - gap)) < 0 and high_count != 0 and low_count == 0:

                    low_list.append(arr[i][col_input])
                    low_no.append(i)
                    ...
                    i = i + 1
                    flag = (avr(i) - avr(i - gap)) * (avr(i + gap) - avr(i))
                    while flag < 0 and (avr(i) - avr(i - gap)) < 0 and low_count != 0 and i < len(arr[:]) - gap - 2:
                        low_list.append(arr[i][col_input])
                        low_no.append(i)
                        low_count = low_count + 1
                    ...
                    if low_count > 1:
                        for j in low_no:
                            if j <= low_no[low_list.index(min(low_list))]:
                                arr[j][col_high] = high
                                arr[j][col_low] = low
                                arr[j][col_prev_high] = high_prev
                                arr[j][col_prev_low] = low_prev
            ...
            for i in range(gap + 3):
                arr[- i - 1][col_high] = high
                arr[- i - 1][col_low] = low
                arr[- i - 1][col_prev_high] = high_prev
                arr[- i - 1][col_prev_low] = low_prev
            return arr
