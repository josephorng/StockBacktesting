...
def signal_type(temp_, total_indices_, CP_td_, OP_td_):
    ...
    for j_ in range(1, len(total_indices_[:])):
        if total_indices_[j_][0] == name_ and total_indices_[j_][2] == last_ and total_indices_[j_][3] == align_y_ \
            ...
            return total_indices_[j_][:] + [avr_5_td_, temp_[14], temp_[15], temp_[16]]
    return ['Empty']
...
total_indices_big = []
total_indices_small = []
with open('./Comparison/Total_list_2000+.csv') as csvfile:
    reader = csv.reader(csvfile)  # change contents to floats
    # each row is a list
    for row in reader:
        total_indices_big.append(row)
with open('./Comparison/Total_list_2000-.csv') as csvfile:
    reader = csv.reader(csvfile)  # change contents to floats
    # each row is a list
    for row in reader:
        total_indices_small.append(row)
...
file_date = y + '-' + m + '-' + day
i = 0
while os.path.isfile('./' + file_date + '_' + str(i) + '.csv'):
    i = i + 1
with open('./' + file_date + '_' + str(i) + '.csv', 'w', newline='') as csvfile:
...
for stock in stock_list:
    if os.path.isfile('./stock_data/DATA/DATA_' + stock + '.csv'):
        ...
        with open('./stock_data/DATA/DATA_' + stock + '.csv') as csvfile:
            ...
            for row in reader:
                temp.append(row)
        ticks = api.ticks(api.Contracts.Stocks[stock], file_date)
        if ticks.volume != [] and ticks.close[0] != [] and len(temp[:]) > 301:
            OP = ticks.close[0]
            CP = ticks.close[-1]
            vol = sum(ticks.volume)
            print('OP = ' + str(OP))
            print('CP = ' + str(CP))
            if vol > 2000:
                signal = signal_type(temp, total_indices_big, CP, OP)
            else:
                signal = signal_type(temp, total_indices_small, CP, OP)
            with open('./' + file_date + '_' + str(i) + '.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([stock, OP, CP, vol] + signal)
